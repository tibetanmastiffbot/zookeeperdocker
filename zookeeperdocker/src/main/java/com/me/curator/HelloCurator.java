...
class ZKCrud {
  ...
  public static String createEphemeralSequential(CuratorFramework client, String path,
    ...
    // this will create the given EPHEMERAL-SEQUENTIAL ZNode with the given data using Curator
    // protection.
     ...
     * creation can succeed on the server, but the server can crash before the created node name is
     * returned to the client. However, the ZK session is still valid so the ephemeral node is not
     * deleted. Thus, there is no way for the client to determine what node was created for them.
     *
     * Even without sequential-ephemeral, however, the create can succeed on the sever but the
     * client (for various reasons) will not know it. Putting the create builder into protection
     * mode works around this. The name of the node that is created is prefixed with a GUID. If node
     * creation fails the normal retry mechanism will occur. On the retry, the parent path is first
     * searched for a node that has the GUID in it. If that node is found, it is assumed to be the
  ...
  public static void guaranteedDelete(CuratorFramework client, String path) throws Exception {
     ...
     * Guaranteed Delete
     *
     * Solves this edge case: deleting a node can fail due to connection issues. Further, if the
     * node was ephemeral, the node will not get auto-deleted as the session is still valid. This
     * can wreak havoc with lock implementations.
     *
     *
     * When guaranteed is set, Curator will record failed node deletions and attempt to delete them
     * in the background until successful. NOTE: you will still get an exception when the deletion
     * fails. But, you can be assured that as long as the CuratorFramework instance is open attempts
